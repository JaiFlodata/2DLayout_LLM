...
Llama 4 via Groq for high‑level reasoning and OpenCV for quick local
...
python pillar_analysis_image.py "Annotated_Office.png" --api-key gsk_... --prefix office
```
...
| `office_pillar_data.json`  | Llama JSON + locally detected pillar metadata   |
"""
...
# -------------------------------------------------------------------- setup
# Optional: load GROQ_API_KEY from .env if python‑dotenv is available
try:
...
class ImagePillarAnalyzer:
    ...
    # -------------------------------------------------------- Llama analysis
    def ask_llama_to_find_pillars(self) -> Dict:
        """Call Llama 4 Scout via Groq with the plan image and get pillar JSON."""
...
{
  "pillar_summary": {
    "total_pillars": <int>,               // number of yellow circles
...
"""
        ...
        try:
            ...
            LOG.info("Raw response from Llama 4 Scout: %s", raw[:200] + "..." if len(raw) > 200 else raw)
            ...
            LOG.error("Llama 4 request failed: %s", exc)
            return {}
    ...
    # ------------------------------------------------------- visualisation
    def visualise(self, llama_json: Dict, prefix: str):
        """Overlay pillar markers, save annotated PNG, report, and JSON."""
        if not llama_json:
            LOG.warning("No Llama data – skipping visualisation.")
            return

        original = self.image
        overlay = original.copy()

        desc = llama_json.get("pillar_analysis", {}).get("pillar_representation", "")
        total_llama = llama_json.get("pillar_analysis", {}).get("total_count", 0)

        local_hits: List[Dict] = []

        # ---- choose detection strategy based on Llama description --------
        ...
        ax1.axis("off"); ax1.set_title(f"Pillars (Llama {total_llama} / local {len(local_hits)})")

        ax2.text(0, 1, json.dumps(llama_json, indent=2), va="top", family="monospace")
        ax2.axis("off")
        ...
        with open(out_path, "w", encoding="utf-8") as fp:
            json.dump({"llama_analysis": llama_json, "local_pillars": local_hits}, fp, indent=2)
        LOG.info("JSON saved → %s", out_path)

    # -------------------------------------------------------------- driver
    def run(self, prefix: str):
        llama_data = self.ask_llama_to_find_pillars()
        self.visualise(llama_data, prefix)
...
def main() -> None:
    ...
    parser.add_argument("--api-key", dest="api_key", default=os.getenv("GROQ_API_KEY"),
                        help="Groq API key (or set GROQ_API_KEY / .env)")
    parser.add_argument("--prefix", default="pillar", help="Output file prefix")
    args = parser.parse_args()

    if not args.api_key:
        parser.error("Groq API key missing – supply --api-key or set GROQ_API_KEY/.env")
